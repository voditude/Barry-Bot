...
- **Transcript-sourced content is a third category, never folded into "original" or "added."**
  When `transcript-slide-extract` surfaces something the lecturer said that isn't on the
  slides, it's genuinely from the lecture — but not from the slides, so it can't honestly wear
  the plain source-box style, and it wasn't generated by the tool, so it can't wear the add-box
  style either. Use the `.transcript-box` style (a full add-on unit — new topic, or a full
  worked example) or the `.from-transcript` inline marker (a short aside folded into the
